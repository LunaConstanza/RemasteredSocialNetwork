...
const firebaseConfig = {
  apiKey: "AIzaSyB7NGkvEvS9BzlxU50_A8yPbVPWiejKVuE",
  authDomain: "socialnetwork2-c9bec.firebaseapp.com",
  projectId: "socialnetwork2-c9bec",
  storageBucket: "socialnetwork2-c9bec.appspot.com",
  messagingSenderId: "956713528463",
  appId: "1:956713528463:web:8568b064ac6739623c94fa"
};
function __rest(s, e) {
  var t = {};
  for (var p2 in s)
...
getStorage(app);
const saveUser = async (userId, displayName, date) => {
...
const dataUser = async () => {
  ...
  querySnapshot.forEach((doc2) => {
    if (doc2.data().uid === auth.currentUser.uid) {
      user.push(doc2.data().displayName);
    }
  });
  return user.join("");
};
const savePost = async (description) => {
...
const editPost = async (id2, description) => {
  const posts = doc(db, "Posts", id2);
  await updateDoc(posts, {
    "description": description
  });
};
const deletePost = async (id2) => {
  await deleteDoc(doc(db, "Posts", id2));
};
const updateLikes = async (id2) => {
  const userIdentifier = auth.currentUser.uid;
  const postRef = doc(db, "Posts", id2);
...
let currentUser;
const registerUser = (email, password, displayName, date) => {
  createUserWithEmailAndPassword(auth, email, password).then((userCredential) => {
    ...
    error2.code;
    error2.message;
    alert("Este correo electr\xF3nico ya existe.");
  });
};
const registerGoogle = (callback) => {
  signInWithPopup(auth, provider).then((result) => {
    ...
    error2.code;
    error2.message;
    error2.customData.email;
    GoogleAuthProvider.credentialFromError(error2);
    callback(false);
  });
};
...
const verification = (auth2) => {
  sendEmailVerification(auth2.currentUser).then(() => {
    alert("Se ha enviado un mensaje de verificaci\xF3n a tu correo electr\xF3nico, por favor revisalo y verifica tu registro. Luego inicia sesi\xF3n.");
  });
};
...
const currentChange = (navigateTo2) => {
...
const closeSesion = (navigateTo2) => {
  signOut(auth).then(() => {
    navigateTo2("/");
  }).catch((error2) => {
  });
};
const img = "/assets/logo.2b1a5c15.png";
const headerLogo = () => {
...
const home = (navigateTo2) => {
  ...
  subTitleLogin.innerHTML = `Inicio de Sesi\xF3n`;
  ...
  userLogin.setAttribute("placeholder", "Ingresa tu correo electr\xF3nico");
  ...
  passwordLogin.setAttribute("placeholder", "Ingresa tu contrase\xF1a");
  ...
  btnLogIn.innerHTML = `<i class="fa-solid fa-right-to-bracket"></i> Iniciar Sesi\xF3n`;
  const textReset = document.createElement("p");
  textReset.innerHTML = `\xBFOlvidaste tu contrase\xF1a? <a href="#">Recup\xE9rala</a>`;
  ...
  h3Popup.innerHTML = `No hay problema \xA1nosotras te ayudamos!`;
  const textPopup = document.createElement("p");
  textPopup.innerHTML = `Enviaremos a tu email un correo para que recuperes tu contrase\xF1a.`;
  const formResetPass = document.createElement("form");
  const inputEmail = document.createElement("input");
  inputEmail.setAttribute("id", "userEmail");
  inputEmail.setAttribute("type", "email");
  inputEmail.setAttribute("placeholder", "Ingresa aqu\xED tu correo electr\xF3nico");
  ...
  resetPassword.innerHTML = `<i class="fa-solid fa-key"></i> Recuperar Contrase\xF1a`;
  const btnGoogle = document.createElement("button");
  btnGoogle.classList.add("btnGoogle");
  btnGoogle.innerHTML = `<i class="fa-brands fa-google"></i> Iniciar sesi\xF3n con Google`;
  const linkRegister = document.createElement("p");
  linkRegister.innerHTML = `\xBFNo tienes cuenta? <a href="/register" id="linkReg">Reg\xEDstrate</a>`;
...
const error = () => {
  ...
  tittle.textContent = "Error 404 page not found";
  return tittle;
};
const register = (navigateTo2) => {
  ...
  inputNameAndLastName.setAttribute("pattern", "[A-Za-z\xC0-\xFF .]{1,25}");
  inputNameAndLastName.setAttribute("title", "- Solo se pueden ingresar letras. M\xE1x. 25 car\xE1cteres.");
  inputNameAndLastName.setAttribute("required", "");
  const inputMail = document.createElement("input");
  inputMail.setAttribute("id", "email");
  inputMail.setAttribute("type", "email");
  inputMail.setAttribute("placeholder", "Correo electr\xF3nico");
  inputMail.setAttribute("required", "");
  const inputPassword = document.createElement("input");
  inputPassword.setAttribute("id", "password");
  inputPassword.setAttribute("type", "password");
  inputPassword.setAttribute("placeholder", "Crear contrase\xF1a");
  ...
  btnSend.setAttribute("value", "Reg\xEDstrate");
...
const dashboard = (navigateTo2) => {
  currentChange(navigateTo2);
  ...
  btnLogOut.innerHTML = `Cerrar sesi\xF3n <i class="fa-solid fa-right-from-bracket"></i>`;
  ...
  titleCreatePost.innerHTML = `\xBFQu\xE9 vas a jugar hoy ?`;
  ...
  inputCreatePost.setAttribute("placeholder", "Escribe aqu\xED...");
    ...
    dataPost().then((res) => {
      res.forEach((doc2) => {
        ...
        html += `
                ...
                <h6>${post.name} public\xF3:</h6>`;
        if (post.uid === auth.currentUser.uid) {
          html += `
                            <div class="mainDash_board_publications_content_user_btns">
                                <button type="button" class="btnEdit" value="${doc2.id}"><i class="fa-solid fa-pen-to-square"></i></button>
                                <button type="button" class="btnDelete" value="${doc2.id}"><i class="fa-solid fa-trash"></i></button>
                            </div>
                        </div>
                        ${img2()}
                        <p id="${doc2.id}" class="mainDash_board_publications_content_text">${post.description}</p>
                        <div class="mainDash_board_publications_content_star">
                            <div class="mainDash_board_publications_content_starDiv">
                                <button class="btn-like mainDash_board_publications_content_starR" value="${doc2.id}">${likes(auth.currentUser.uid)}</button>
      ...
      btnDelete.forEach((btn) => {
        btn.addEventListener("click", () => {
          console.log("click");
          if (confirm("\xBFEst\xE1s segura de eliminar esta publicaci\xF3n?")) {
